...
import (
	...
	"github.com/liujiyong7/smaug/internal/encoder/alg"
	"github.com/liujiyong7/smaug/internal/encoder/vars"
	"github.com/liujiyong7/smaug/internal/rt"
	"github.com/liujiyong7/smaug/option"
	"github.com/liujiyong7/smaug/utf8"
)
...
// 编译入口
func CompileBinaryAndStore(program *vm.Program) error {
	if program == nil {
		return fmt.Errorf("program is nil")
	}
	efv := rt.UnpackEface(program)
	_, err := findOrCompile(efv.Type, program)
	return err
}

// 运行入口
func FindBinaryAndRun(program *vm.Program, env any) (any, error) {
	if program == nil {
		return nil, fmt.Errorf("program is nil")
	}
	efv := rt.UnpackEface(program)
	if fn, err := findOrCompile(efv.Type, program); err != nil {
		return nil, err
	} else {
		fn(nil, nil, 0)
	}
...
var testFunc _BinaryProgram = func(
	vp unsafe.Pointer,
	sb *_Stack,
	fv uint64,
) error {
...
func makeBinaryProgram(vt *rt.GoType, ex ...interface{}) (interface{}, error) {
	program := ex[0].(*vm.Program)
	if pp, err := newCompiler().compile(program); err != nil {
		return nil, err
	} else {
		as := newAssembler(pp)
		as.name = vt.String()
...
func findOrCompile(vt *rt.GoType, program *vm.Program) (_BinaryProgram, error) {
	if val := programCache.Get(vt); val != nil {
		return val.(_BinaryProgram), nil
	} else if ret, err := programCache.Compute(vt, makeBinaryProgram, program); err == nil {
		return ret.(_BinaryProgram), nil
	} else {
		return nil, err
	}
}
